...
{{
  ...
  "image_prompt": "МАКСИМАЛЬНО КОНКРЕТНОЕ описание медицинского объекта/предмета/процесса на русском языке (например: 'блистер с белыми таблетками на голубом фоне, крупный план' ИЛИ 'пробирки с кровью в лабораторном штативе' ИЛИ 'свежие овощи на тарелке, вид сверху')",
...
- `image_prompt` — МАКСИМАЛЬНО КОНКРЕТНОЕ описание НА РУССКОМ ЯЗЫКЕ для фотореалистичного медицинского изображения
- КРИТИЧЕСКИ ВАЖНО: Промпт должен быть ПРЕДЕЛЬНО СПЕЦИФИЧНЫМ и отражать ТОЧНУЮ суть статьи
- ОБЯЗАТЕЛЬНЫЕ элементы промпта:
  1) Конкретный объект/предмет (что именно изображено)
  2) Медицинский контекст (связь с темой статьи)
  3) Визуальные детали (цвет, форма, расположение)
  4) Стиль съемки (крупный план, студийное фото, макро и т.д.)

- Варианты в зависимости от ТЕМЫ статьи (ИСПОЛЬЗУЙ КАК ШАБЛОН, АДАПТИРУЙ ПОД КОНКРЕТНУЮ ТЕМУ):
  * Лекарства/препараты → "блистер с [название препарата/тип таблеток] белого цвета на светло-голубом медицинском фоне, крупный план, студийная съемка"
  * Питание/диета → "тарелка со свежими [конкретные продукты] на белом столе, вид сверху, естественное освещение, здоровое питание"
  * Симптомы/болезни → "анатомическая модель [конкретный орган/система] из медицинского учебника, профессиональная фотография, реалистичные детали"
  * Диагностика → "[конкретное диагностическое оборудование] в современной клинике, профессиональное медицинское оборудование, детальный вид"
  * Профилактика → "человек выполняет [конкретное упражнение/действие] на открытом воздухе, концепция здорового образа жизни"
  * Исследования → "научная лаборатория с [конкретное оборудование: микроскоп/пробирки/чашки Петри], медицинские исследования, профессиональная съемка"
  * Вакцины/прививки → "одноразовый шприц с прозрачной жидкостью и флакон вакцины на медицинском столе, крупный план"
  * Витамины/БАДы → "разноцветные капсулы и таблетки [конкретного типа] рассыпанные на белой поверхности, макросъемка"
  * Беременность → "беременная женщина [конкретное действие: на УЗИ/держит животик] в светлом помещении, естественное фото"
  * Детское здоровье → "здоровый ребенок [возраст и действие] с улыбкой, естественное освещение, семейная атмосфера"
  * Хирургия/операции → "хирургические инструменты [конкретные] на стерильном столе, медицинская обстановка"
  * Анализы/диагностика → "пробирки с кровью в лабораторном штативе, медицинский анализ, профессиональное оборудование"

- СТРОГИЕ ПРАВИЛА:
  ✅ ОБЯЗАТЕЛЬНО: Конкретные предметы/объекты/процессы, напрямую связанные с темой
  ✅ ОБЯЗАТЕЛЬНО: Добавляй визуальные детали (цвет, материал, фон, освещение)
  ✅ ОБЯЗАТЕЛЬНО: Указывай стиль съемки (крупный план, макро, студийное фото, вид сверху)
  ❌ ИЗБЕГАЙ: Общие фразы типа "медицинская сцена", "врач", "больница", "клиника"
  ❌ ИЗБЕГАЙ: Врачей в халатах, медицинских кабинетов, больничных интерьеров (ЕСЛИ это не суть статьи)
  ❌ ИЗБЕГАЙ: Абстрактных концепций без конкретных объектов

- ПРИМЕРЫ ПРАВИЛЬНЫХ ПРОМПТОВ:
  ✅ "блистер с белыми овальными таблетками метформина на светло-сером медицинском фоне, крупный план, студийная съемка"
  ✅ "свежие листья зеленого салата, помидоры черри и авокадо на белой тарелке, вид сверху, здоровое питание"
  ✅ "анатомическая модель женской репродуктивной системы с яичниками из медицинского учебника, профессиональная фотография"
  ✅ "современный аппарат УЗИ с датчиком в гинекологическом кабинете, профессиональное медицинское оборудование"

- ПРИМЕРЫ НЕПРАВИЛЬНЫХ ПРОМПТОВ:
  ❌ "медицинская тема" (слишком общо)
  ❌ "врач осматривает пациента" (не специфично для темы)
  ❌ "больница" (не относится к сути статьи)
  ❌ "здоровье" (слишком абстрактно)
...
{{
  ...
  "seo_description": "SEO описание до 160 символов",
  "seo_keywords": ["ключевое_слово_1", "ключевое_слово_2", "ключевое_слово_3"],
  "image_prompt": "МАКСИМАЛЬНО КОНКРЕТНОЕ описание медицинского объекта/предмета/процесса на русском языке с визуальными деталями (цвет, фон, ракурс). Примеры: 'блистер с белыми овальными таблетками на светло-сером фоне, крупный план, студийная съемка' ИЛИ 'пробирки с красной кровью в металлическом лабораторном штативе, медицинский анализ' ИЛИ 'тарелка со свежими зелеными овощами и помидорами черри на белом столе, вид сверху'",
  "image_url": "https://example.com/image.jpg"
}}"""
