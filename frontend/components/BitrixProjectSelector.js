...
const BitrixProjectSelector = ({ onProjectChange, selectedProject }) => {
  ...
  const fetchProjects = async () => {
    try {
      const response = await fetch('/api/settings/bitrix-projects')
      ...
      if (response.ok) {
        ...
        data.forEach(project => {
          if (project.is_active) {
            platformsData[project.project_code] = {
              name: project.display_name,
              style: "educational",
              topics: [], // Можно добавить topics из описания
              api_url: project.api_url,
              api_token: project.api_token
            }
          }
        })
        setProjects(platformsData)
      } else {
      ...
      {selectedProject && (
        <div className="mt-4 p-4 bg-blue-50 rounded-lg">
          <p className="text-sm text-blue-800">
            <strong>Выбран проект:</strong> {projects[selectedProject]?.name}
          </p>
...
export default BitrixProjectSelector